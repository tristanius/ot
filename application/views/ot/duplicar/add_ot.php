...
<table class="mytabla" ng-init="getItemsBy('<?= site_url('Ot/getDataNewForm') ?>')">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Orden Basica: <input type="checkbox" ng-model="ot.basica"  ng-false-value="'0'"  ng-true-value="'1'"/></td>
      <td colspan="4">
  ...
  <tbody>
    <tr>
      ...
      <td>
        <select class="noMaterialStyles" ng-model="ot.sistema_ecp">
...
<div class="row">
  ...
  <section class="row">
    ...
    <div class="col s6 m2 l2" style="border:1px solid #CCC; padding:3px;">
      <h6>Resumen:</h6>
      <button class="btn blue darken-4 orange mini-btn2" ng-click="getResumenGeneral('<?= site_url('ot/resumenItems/'.$ot->idOT) ?>')">resumen</button>
    </div>
  </section>
</div>
...
<!-- opciones -->
<div class="btnWindow">
  <button type="button" class="waves-effect waves-light btn" ng-click="guardarOT('<?= site_url('ot/saveOT/duplicated') ?>')"> Finalizar</button>
